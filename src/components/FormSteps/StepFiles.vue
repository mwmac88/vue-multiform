<template>
  <v-form ref="formstep2">
    <v-flex v-for="(input, key, index) in formInputs.step2" :key="index" xs12 ma-3>
      <v-btn v-if="input.type === 'file'" @click="openFileDialog(key)">
        {{ input.btnlabel }}
...
export default {
  ...
  computed: {
    ...
    uploadedFiles() {
      return Object.values(this.formInputs.step2).filter(files =>
        Boolean(files.value)
      );
    },
    hasUploadedCV() {
